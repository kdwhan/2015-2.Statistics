...
* Description: 두개의 dataset을 사용하여 Stem 과 leaf를 양쪽으로 출력해 주는 프로그램이다.
				십의 자리 숫자를 줄기(stem)으로 하여 작은 수부터 순서대로 가운데 나열하고
				양쪽에는 각 줄기에 해당하는 각 dataSet의 일의 자리 숫자를 잎(leaf)으로 가로로 나열한다.
...
* Description: 퀵정렬 함수이다. partition함수를 사용하여 범위를 나누고 합치며 정렬 해 나간다.
				정렬할 데이터셋과 시작 인덱스와 마지막 인덱스를 인자로 받는다.
*******************************************************************************/
...
static DataSet *CreateDataSet(const char *fileName)
{
	...
	end = out->elem + out->length;// end를 elem의 끝 주소로 초기화
...
static void PrintStemPlotList(StemPlotList *leftPlotList, StemPlotList *rightPlotList)
{
	...
	while ((leftPlotList->head != NULL) && (rightPlotList->head != NULL))
	{
		...
		if (leftPlotList->head->stem == rightPlotList->head->stem) // 같은 stem 일 경우
		{
			len = 0;
		...
		else if (leftPlotList->head->stem < rightPlotList->head->stem)// left의 스텀이 작은경우
		{
			len = 0;
			for (i = leftPlotList->head->freq - 1; i >= 0; i--)
			{
	...
	if (leftPlotList->head == NULL && rightPlotList->head != NULL) // 다 했는데 오른쪽 data 가 남았을 경우
	{
	...
	else if (leftPlotList->head != NULL && rightPlotList->head == NULL) // 왼쪽이 남아있을 경우
	{
